...
class CreateEventViewController: UIViewController, ShowAlert, GMSPlacePickerViewControllerDelegate, CreateEventProfileViewControllerDelegate {
    ...
    internal func validation() -> Bool {
        ...
        if (createEventInfoParams["EventTitle"] as? String ?? "").characters.count <= 0 {
            
            self.showAlert(AKErrorHandler.CreateEvent.titleEmpty)
            
        } else if (createEventInfoParams["EventSubTitle"] as? String ?? "").characters.count <= 0 {
            
            self.showAlert(AKErrorHandler.CreateEvent.SubtitleEmpty)
            
        } else if (createEventInfoParams["EventPlaceName"] as? String ?? "").characters.count <= 0 &&  (createEventInfoParams["EventPlaceAddress"] as? String ?? "").characters.count <= 0 {
            
            self.showAlert(AKErrorHandler.CreateEvent.addLocation)
            
        }else if (createEventInfoParams["EventStartTime"] as? String ?? "").characters.count <= 0 {
            
            self.showAlert(AKErrorHandler.CreateEvent.startTime)
            
        } else if (createEventInfoParams["EventStopTime"] as? String ?? "").characters.count <= 0 {
            
            self.showAlert(AKErrorHandler.CreateEvent.endTime)
            
        } else if (createEventInfoParams["EventDescription"] as? String ?? "").characters.count <= 0 {
...
extension CreateEventViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    internal func getTableViewCellCreateEventTextField(tableView: UITableView, indexPath: IndexPath) -> TableViewCellCreateEventTextField {
        ...
        if indexPath.row == 0 {
            ...
            if (createEventInfoParams["EventPlaceName"] as? String ?? "").characters.count == 0 || (createEventInfoParams["EventPlaceAddress"] as? String ?? "").characters.count == 0 {
    ...
    internal func getTableViewForDonationLocation(tableView: UITableView, indexPath: IndexPath) -> TableViewCellCreateEventTextField {
        ...
        if (createEventInfoParams["causeDonationPlaceName"] as? String ?? "").characters.count == 0 || (createEventInfoParams["causeDonationPlaceAddress"] as? String ?? "").characters.count == 0 {
