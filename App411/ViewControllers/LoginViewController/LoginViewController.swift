...
extension UIButton {
    ...
    var stringProperty:UIEdgeInsets {
        get {
            return objc_getAssociatedObject(self, &AssociatedObjectHandle) as! UIEdgeInsets
        }
        set {
...
class LoginViewController: UIViewController, ShowAlert {
    ...
    internal func validation() -> Bool {
        ...
        if (params["email"]?.characters.count)! <= 0 && (params["password"]?.characters.count)! <= 0 {
            
            self.showAlert(AKErrorHandler.CommonErrorMessages.Empty_Email_Password)
            
        } else if (params["email"]?.characters.count)! <= 0 {
            
            self.showAlert(AKErrorHandler.CommonErrorMessages.Empty_Email)
            
        } else if (params["password"]?.characters.count)! <= 0 {
...
extension LoginViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if section == 0 {
            ...
            label_Title.text = "Log in to 411"
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        if textField.tag == 0 {
            ...
            if let textFiledText: String = textField.text {
                ...
                if textFiledText.characters.count > 0 {
